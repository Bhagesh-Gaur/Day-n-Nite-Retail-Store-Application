-- DROP DATABASE mid_evaluation_grp5;
...
Create table Customer(
    ...
    Email_ID varchar(240) not null primary key CHECK (Email_ID REGEXP "^[a-zA-Z0-9+_.-]+@[a-zA-Z0-9.-]+$") UNIQUE
) AUTO_INCREMENT=1;
...
Create table Customer_Phone(
	phone_number varchar(10) not null CHECK (phone_number REGEXP "^[1-9][0-9]+$" AND LENGTH(phone_number) = 10),
...
Create table Employee(
    ...
    Email_ID varchar(240) CHECK (Email_ID REGEXP "^[a-zA-Z0-9+_.-]+@[a-zA-Z0-9.-]+$") UNIQUE,
    BranchID integer,
    Date_of_join date,
...
Create table Supplier_Phone(
	SupplierID integer,
	phone_number varchar(10) CHECK (phone_number REGEXP "^[1-9][0-9]+$" AND LENGTH(phone_number) = 10),
...
create table Offline_Purchase(
    ...
    Total_Charge decimal not null CHECK( Total_Charge >= 0),
    BranchID integer,
    CustomerID varchar(240),
    foreign key(CustomerID) references Customer(Email_ID),
    foreign key(BranchID) references Branch(BranchID)
...
create table UPI(
	UPI_ID varchar(45) not null UNIQUE,
    PaymentID integer,
    foreign key(PaymentID) references Payment_Options(PaymentID)
);
